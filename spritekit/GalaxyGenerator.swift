...
enum GalaxyGenerator {
    ...
    static func generate(config: GalaxyConfig = GalaxyConfig(planetCount: 10, seed: 0)) -> GameState {
        ...
        var planets = generatePlanets(count: count, rng: &rng)
        assignFactions(to: &planets, count: count)

        let routes = generateRoutes(for: planets, rng: &rng)
    ...
    private static func generatePlanets(count: Int, rng: inout SeededRNG) -> [Planet] {
    ...
    // MARK: - Faction Assignment

    private static func assignFactions(to planets: inout [Planet], count: Int) {
        // Player home
        ...
        let indices = spreadIndices(count: aiFactions.count, total: count, avoiding: 0)
        for (i, factionIndex) in indices.enumerated() where i < aiFactions.count {
    ...
    private static func generateRoutes(for planets: [Planet], rng: inout SeededRNG) -> [Route] {
